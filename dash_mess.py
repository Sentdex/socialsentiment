...
# One SQLite connection - OK - select statements only & no serialization
...
app.layout = html.Div(
    [html.Div(className='container-fluid', children=[html.H2('Live Twitter Sentiment', style={'color': "#CECECE"}),
     ...
     html.Div(className='row', children=[
         html.Div(id='related-sentiment', children=html.Button('Loading related terms...', id='related_term_button'),
...
# Counter with blacklist words
# Empty word with big value - will serve to filter counter
stop_words.append('')
blacklist_counter = Counter(dict(zip(stop_words, [1000000]*len(stop_words))))

# Compile regex for split operation - punctuation list, plus space & new line
...
def related_sentiments(df, sentiment_term, how_many=15):
    try:
        ...
        # Speed optimization:
        # - Join strings to one string
        # - Use precompiled regex to split using punctuation list plus space and new line characters
        tokens = split_regex.split(' '.join(df['tweet'].values.tolist()).lower())

        # Speed optimization:
        # - Remove stop_words, sentiment_term, empty_token
        # - Make another counter with a big value and subtracting
        # - Counter will subtract and remove tokens with negative count
...
def update_hist_graph_scatter(sentiment_term):
    try:
        if sentiment_term:
            df = pd.read_sql(
                "SELECT sentiment.* FROM sentiment_fts fts LEFT JOIN sentiment ON fts.rowid = sentiment.id WHERE fts.sentiment_fts MATCH ? ORDER BY fts.rowid DESC LIMIT 10000",
                conn, params=(sentiment_term + '*',))
        else:
